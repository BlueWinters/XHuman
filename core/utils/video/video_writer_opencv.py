...
class XVideoWriterOpencv:
    ...
    def reformatVideoTo(path_video_source, suffix='.mp4'):
        ...
        XVideoWriterOpencv.reformatVideo(path_video_source, path_video_target)
    ...
    def isForceDefaultFormat(self):
        if platform.system().lower() == 'windows':
            return False
        return XVideoWriterOpencv.ConfigForceDefaultFormat
    ...
    def _config(self, config: dict):
        self.fps = config['fps'] if 'fps' in config \
            else XVideoWriterOpencv.default_FPS()
        self.fourcc = config['fourcc'] if 'fourcc' in config \
            else XVideoWriterOpencv.default_fourcc()
    ...
    def _getWriter(path, fps, w, h, fourcc, backend='opencv'):
        is_open, video_writer = XVideoWriterOpencv._getOpencvWriter(path, fps, w, h, fourcc)
        if is_open is False:
    ...
    def open(self, path: str):
        if hasattr(self, 'path') is False:
            ...
            if XVideoWriterOpencv.isForceDefaultFormat:
                self.path = '{}{}'.format(os.path.splitext(self.path)[0], XVideoWriterOpencv.default_suffix())
...
class XVideoWriterOpencvSynchronous(XVideoWriterOpencv):
    """
    """
    def __init__(self, config):
        super(XVideoWriterOpencvSynchronous, self).__init__(config)
...
class XVideoWriterOpencvAsynchronous(XVideoWriterOpencv):
    """
    """
    def __init__(self, config):
        super(XVideoWriterOpencvAsynchronous, self).__init__(config)
