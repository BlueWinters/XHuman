...
class LibHeadPose:
    ...
    def visual(bgr, radians, landmarks):
        for radian, points in zip(radians, landmarks):
            ...
            size = (bgr.shape[0] + bgr.shape[1]) // 10
            LibHeadPose.plot_axis(bgr, yaw, pitch, roll, cx=cx, cy=cy, size=size)
        return bgr
    ...
    """
    """
    def _clip_and_resize(self, bgr, lft, rig, top, bot, dH, dW):
        H, W, C = bgr.shape
    ...
    def _calculate_outer_with_68pts(self, pts, ext: float):
    ...
    def _format(self, bgr, pts68, ext:float=0.3):
        assert isinstance(pts68, np.ndarray)
        assert pts68.shape == (68, 2)
        lft, rig, top, bot = self._calculate_outer_with_68pts(pts68, ext=ext)
        bgr_rsz = self._clip_and_resize(bgr, lft, rig, top, bot, self.H, self.W)
        return bgr_rsz

    def _forward(self, bgr_rsz):
    ...
    def _estimateLandmark(self, bgr, landmarks):
    ...
    def inference(self, bgr, landmarks=None):
        landmarks = self._estimateLandmark(bgr, landmarks)
        radians = np.zeros(shape=(len(landmarks), 3), dtype=np.float32)
        for n in range(len(landmarks)):
            bgr_rsz = self._format(bgr, landmarks[n, :, :])
            yaw, pitch, roll = self._forward(bgr_rsz)
    ...
    def _returnResult(self, bgr, landmarks, radians, targets):
    ...
    def __call__(self, bgr, *args, **kwargs):
        ...
        return self._returnResult(bgr, landmarks, radians, targets)
