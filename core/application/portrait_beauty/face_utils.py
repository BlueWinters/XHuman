...
def get_reference_facial_points(output_size=None,
    ...
    if (inner_padding_factor == 0 and outer_padding == (0, 0)):
        if output_size is None:
            logging.info('No paddings to do: return default reference points')
            return ref_5pts
        else:
    ...
    if ((inner_padding_factor > 0 or outer_padding[0] > 0
        ...
        logging.info('deduced from paddings, output_size = ', output_size)
...
def smooth_mask(mask, mask_size, mask_blur):
    convert = False
    if mask_size > 0:
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (mask_size, mask_size))
        mask = cv2.dilate(mask, kernel)
    elif mask_size < 0:
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (np.abs(mask_size), np.abs(mask_size)))
