...
class LibTennisBallBounceDetector:
    """
    """
    Config = {
        'model_path': 'thirdparty/tennis/tennis_ctb_regr_bounce.cbm',
    }
    ...
    def initialize(self, *args, **kwargs):
        ...
        path = '{}/{}'.format(root, self.Config['model_path'])
    ...
    def predict(self, ball_track, smooth=True):
    ...
    def __call__(self, ball_track, *args, **kwargs):
        targets, inference_kwargs = self.extractArgs(*args, **kwargs)
        bounces = self.predict(ball_track)
        return self.returnResult(bounces, targets)
