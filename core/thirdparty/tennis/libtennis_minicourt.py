...
class LibTennisMiniCourt:
    ...
    def __new__(cls, *args, **kwargs):
        if cls.Instance is None:
            cls.Instance = super(LibTennisMiniCourt, cls).__new__(cls, *args, **kwargs)
        return cls.Instance
    ...
    def __init__(self):
        ...
        key_points = [
            ...
            *self.middle_line]
        self.key_points = np.array(key_points, dtype=np.float32).reshape((-1, 1, 2))
        ...
        self.court_conf_ind = {}
        for i in range(len(self.court_conf)):
            conf = self.court_conf[i + 1]
            inds = []
            for j in range(4):
                inds.append(key_points.index(conf[j]))
            self.court_conf_ind[i + 1] = inds
        self.minimap_width = 166
        self.minimap_height = 350

    def build_court_reference(self):
        """
        Create court reference image using the lines positions
        """
        shape = (self.court_height + 2 * self.top_bottom_border, self.court_width + 2 * self.right_left_border)
        court = np.zeros(shape, dtype=np.uint8)
        ...
        court_bgr = (np.stack((court, court, court), axis=2) * 255).astype(np.uint8)
        return court_bgr
